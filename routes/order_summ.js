...
module.exports = (db) => {
  router.get("/", (req, res) => {
    ...
    db.query(`SELECT food.name as food_name, order_food.quantity, order_food.price, users.phone_number FROM order_food JOIN food ON food.id = food_id, users WHERE order_food.order_id = 5 AND users.id = ${id};`)
      .then(foodOrders => {
        ...
        //const foodDuration = orderSummary.rows[0].time;
